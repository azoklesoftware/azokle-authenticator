...
import com.azokle.authenticator.otp.SteamInfo;
import com.azokle.authenticator.otp.TotpInfo;
import com.azokle.authenticator.util.UUIDMap;
import com.azokle.authenticator.vault.VaultEntry;
import com.azokle.authenticator.vectors.VaultEntries;
...
public class DatabaseImporterTest {
     ...
     * -> while read line; do (qrencode "$line" -o - | feh -); done <
     * ./app/src/test/resources/com/azokle/azokle-auth/importers/plain.txt
    ...
    @Test
    public void testImportAzokleAuthEncrypted() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(AzokleAuthImporter.class, "azokle_auth_encrypted.json", encryptedState -> {
            final char[] password = "test".toCharArray();
            return ((AzokleAuthImporter.EncryptedState) encryptedState).decrypt(password);
        });
    ...
    public void testImportAndOTPEncryptedOld() throws IOException, DatabaseImporterException, OtpInfoException {
        ...
        for (VaultEntry entry : entries) {
            // old versions of andOTP have a bug where the issuer/name is not parsed
            // correctly, so account for that here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
    ...
    public void testImportTotpAuthenticator() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(TotpAuthenticatorImporter.class, "totp_authenticator.bin",
                encryptedState -> {
    ...
    public void testImportTotpAuthenticatorInternal() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importPlain(TotpAuthenticatorImporter.class, "totp_authenticator_internal.xml",
                true);
        checkImportedTotpAuthenticatorEntries(entries);
    }
    ...
    public void testImportAuthProEncryptedLegacy() throws DatabaseImporterException, IOException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(AuthenticatorProImporter.class, "authpro_encrypted_legacy.bin",
                state -> {
                    char[] password = "test".toCharArray();
    ...
    public void testImportGoogleAuthenticator() throws IOException, DatabaseImporterException, OtpInfoException {
        ...
        for (VaultEntry entry : entries) {
            // Google Authenticator doesn't support different hash algorithms, periods or
            // digits, so fix those up here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
    ...
    public void testImportMicrosoftAuthenticator() throws IOException, DatabaseImporterException, OtpInfoException {
        ...
        for (VaultEntry entry : entries) {
            // Microsoft Authenticator doesn't support HOTP, different hash algorithms,
            // periods or digits, so fix those up here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
    ...
    public void testImportAuthenticatorPlus() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(AuthenticatorPlusImporter.class, "authenticator_plus.zip",
                encryptedState -> {
    ...
    public void testImportTwoFASAuthenticatorSchema1() throws DatabaseImporterException, IOException, OtpInfoException {
        ...
        for (VaultEntry entry : entries) {
            // 2FAS Authenticator schema v1 doesn't support HOTP, different hash algorithms,
            // periods or digits, so fix those up here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
            entryVector.setInfo(new TotpInfo(entryVector.getInfo().getSecret()));
            checkImportedEntry(entryVector, entry);
    ...
    public void testImportTwoFASAuthenticatorSchema2Encrypted()
            throws DatabaseImporterException, IOException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(TwoFASImporter.class, "2fas_authenticator_encrypted.2fas",
                encryptedState -> {
                    final char[] password = "test".toCharArray();
    ...
    public void testImportTwoFASAuthenticatorSchema3Encrypted()
            throws DatabaseImporterException, IOException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(TwoFASImporter.class, "2fas_authenticator_encrypted_v3.2fas",
                encryptedState -> {
                    final char[] password = "test".toCharArray();
                    return ((TwoFASImporter.EncryptedState) encryptedState).decrypt(password);
                });
    ...
    @Test
    public void testImportTwoFASAuthenticatorSchema4Plain()
            throws DatabaseImporterException, IOException, OtpInfoException {
    ...
    @Test
    public void testImportTwoFASAuthenticatorSchema4Encrypted()
            throws DatabaseImporterException, IOException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(TwoFASImporter.class, "2fas_authenticator_encrypted_v4.2fas",
                encryptedState -> {
                    final char[] password = "test".toCharArray();
                    return ((TwoFASImporter.EncryptedState) encryptedState).decrypt(password);
                });
    ...
    private List<VaultEntry> importEncrypted(Class<? extends DatabaseImporter> type, String resName,
            Decryptor decryptor)
            throws IOException, DatabaseImporterException {
        return importEncrypted(type, resName, false, decryptor);
    }

    private List<VaultEntry> importEncrypted(Class<? extends DatabaseImporter> type, String resName, boolean isInternal,
            Decryptor decryptor)
            throws IOException, DatabaseImporterException {
        Context context = ApplicationProvider.getApplicationContext();
        DatabaseImporter importer = DatabaseImporter.create(context, type);
    ...
    private void checkImportedTwoFASSchema2Entries(List<VaultEntry> entries) throws OtpInfoException {
        for (VaultEntry entry : entries) {
            // 2FAS Authenticator doesn't support certain features, so fix those entries up
            // here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
    ...
    private void checkImportedAuthyEntries(List<VaultEntry> entries) throws OtpInfoException {
        for (VaultEntry entry : entries) {
            // Authy doesn't support different hash algorithms or periods, so fix those up
            // here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
            entryVector.getInfo().setAlgorithm(OtpInfo.DEFAULT_ALGORITHM);
    ...
    private void checkImportedTotpAuthenticatorEntries(List<VaultEntry> entries) throws OtpInfoException {
        for (VaultEntry entry : entries) {
            // TOTP Authenticator doesn't support different hash algorithms, periods or
            // digits, so fix those up here
            VaultEntry entryVector = getEntryVectorBySecret(entry.getInfo().getSecret());
            entryVector.getInfo().setDigits(OtpInfo.DEFAULT_DIGITS);
            ((TotpInfo) entryVector.getInfo()).setPeriod(TotpInfo.DEFAULT_PERIOD);
    ...
    private void checkImportedEntry(VaultEntry entryVector, VaultEntry entry) throws OtpInfoException {
        String message = String.format("Entries are not equivalent: (%s) (%s)", entryVector.toJson().toString(),
