...
Azokle Auth persists the user's token secrets and related information to a file. This
...
This document describes Azokle Auth's security design and file format. It's split up
...
Two cryptographic primitives were selected for use in Azokle Auth. An Authenticated
...
Azokle Auth vault also relies on the assumption that this limit is never exceeded.
This is a reasonable assumption to make, because it's highly unlikely that an
Azokle Auth user will ever come close to saving the vault 2<sup>32</sup> times.
...
Azokle Auth supports unlocking a vault with multiple different credentials. The main
...
vault](/app/src/test/resources/com/azokle/authenticator/importers/azokle_plain.json)
and an [encrypted
vault](/app/src/test/resources/com/azokle/authenticator/importers/azokle_encrypted.json)
are available in the [test
data](/app/src/test/resources/com/azokle/authenticator/importers) folder.
There's also an example Python script that can decrypt an Azokle Auth vault given the
...
encrypted and Azokle Auth will try to parse it as such.
...
the same as TOTP, but it uses a different final encoding step. Azokle Auth's
implementation of it can be found in
[crypto/otp/OTP.java](/app/src/main/java/com/azokle/authenticator/crypto/otp/OTP.java).
...
There is no specification available for MOTP. Azokle Auth's implementation of it can be
found in
[crypto/otp/MOTP.java](/app/src/main/java/com/azokle/authenticator/crypto/otp/MOTP.java).
...
There is no specification available for Yandex's OTP algorithm. Azokle Auth's
implementation can be found in
[crypto/otp/YAOTP.java](/app/src/main/java/com/azokle/authenticator/crypto/otp/YAOTP.java)
